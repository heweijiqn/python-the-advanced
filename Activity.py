...
thisDate = datetime.date.today() - datetime.timedelta(days=0)  # 前几天的日期，今天、昨天、前天分别取days=0、1、2
print(thisDate)
...
if NewRow < 8888:  # 由MyAction工作表生成当日活动
    ...
    for j in range(2, 32 - iCnt):
        ...
        sht.range(NewRow, 4).value = (actionSht.range(j, 4).value // 5) * 5  # 时长
        NewRow += 1
